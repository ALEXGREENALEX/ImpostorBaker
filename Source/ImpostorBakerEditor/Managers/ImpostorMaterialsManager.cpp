...
void UImpostorMaterialsManager::Initialize()
{
	...
	if (const TSoftObjectPtr<UMaterialInterface>* WeakNormalMaterial = GetDefault<UImpostorBakerSettings>()->BufferPostProcessMaterials.Find(EImpostorBakeMapType::Normal))
	{
		if (!WeakNormalMaterial->IsNull())
		{
			NormalMaterial = UMaterialInstanceDynamic::Create(WeakNormalMaterial->LoadSynchronous(), nullptr);
			check(NormalMaterial);
		}
	}
...
{
	SampleFrameMaterial->SetScalarParameterValue("UseSRGBBaseColor", TargetMap == EImpostorBakeMapType::BaseColor ? 1.0f : 0.0f);

	if (TargetMap == EImpostorBakeMapType::BaseColor)
	{
		if (ImpostorData->bUseDistanceFieldAlpha)
		{
			SampleFrameMaterial->SetScalarParameterValue("UseDistanceField", 1.0f);
		}
		else
		{
			SampleFrameMaterial->SetScalarParameterValue("UseDistanceField", 0.0f);
		}

		SampleFrameMaterial->SetScalarParameterValue(FName("Dilation"), ImpostorData->bEnableDilation ? 1.0f : 0.0f);
	...
	{
		SampleFrameMaterial->SetScalarParameterValue(FName("Dilation"), 0.0f);
		SampleFrameMaterial->SetScalarParameterValue("UseDistanceField", 0.0f);
	}
...
UMaterialInterface* UImpostorMaterialsManager::GetRenderTypeMaterial(const EImpostorBakeMapType TargetMap) const
{
	...
	case EImpostorBakeMapType::Subsurface:
		return GetDefault<UImpostorBakerSettings>()->BufferPostProcessMaterials.FindRef(TargetMap).LoadSynchronous();
	}
}
...
bool UImpostorMaterialsManager::HasRenderTypeMaterial(const EImpostorBakeMapType TargetMap) const
{
	...
	case EImpostorBakeMapType::Normal:
		return NormalMaterial != nullptr;

	case EImpostorBakeMapType::Depth:
		return DepthMaterial != nullptr;
	...
	case EImpostorBakeMapType::Opacity:
	case EImpostorBakeMapType::Subsurface:
		return GetDefault<UImpostorBakerSettings>()->BufferPostProcessMaterials.FindRef(TargetMap).LoadSynchronous() != nullptr;
	}
}
...
{
	SampleFrameMaterial->SetTextureParameterValue(FName("SRGBBaseColor"), GetManager<UImpostorRenderTargetsManager>()->SceneCaptureSRGBMip);
	SampleFrameMaterial->SetTextureParameterValue(FName("LinearBaseColor"), GetManager<UImpostorRenderTargetsManager>()->SceneCaptureMipChain[0]);
...
void UImpostorMaterialsManager::UpdateBaseColorCustomLightingMaterial() const
{
	...
	BaseColorCustomLightingMaterial->SetTextureParameterValue(FName("BaseColor"), RenderTargetsManager->BaseColorScratchRenderTarget);
